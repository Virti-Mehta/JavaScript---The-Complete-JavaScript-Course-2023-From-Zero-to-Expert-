...
poll.registerNewAnswer = function () {
  const answer = Number(
    prompt(
      // `${this.question}\n${this.options.toString().replaceAll(',', '\n')})`
      `${this.question}\n${this.options.join('\n')}\nWrite option number:)`
    )
  );
  // if (typeof answer === 'number' && answer < this.options.length) {
  //   this.answers[answer]++;
  // } We can apply short circuing of &&
  typeof answer === 'number' &&
    answer < this.options.length &&
    this.answers[answer]++;
  this.displayResults();
  this.displayResults('string');
  // return (displayResults = function (type = 'array') {
  //   if (type === 'array') {
  //     console.log(this.answers);
  //   } else if (type === 'string') {
  //     this.result = this.answers.toString().replaceAll(',', ', ');
  //     console.log(`Poll results are ${this.result}`);
  //   }
  // });
};
...
§ Data 2: [1, 5, 3, 9, 6, 1]
*/
// const displayBonus = poll.displayResults.bind({ answers: [5, 2, 3] });
// const displayBonus = poll.displayResults.bind({ answers: [1, 5, 3, 9, 6, 1] });
// displayBonus();
// displayBonus('string');
